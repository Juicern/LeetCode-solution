...
public class Solution
{
    ...
    {
        if (Char.IsNumber(S[0]) || S[0] == '(' || S[0] == '+')
        {
            var stripped = RemoveSpecialCharacters(S);
            var charsToMask = stripped.Length - 4;

            return String.Concat(
                stripped.Length > 10 ? "+" : "",
                new string('*', charsToMask % 3),
                charsToMask % 3 > 0 ? "-" : "",
                string.Concat(Enumerable.Repeat("***-", charsToMask / 3)),
                stripped.Substring(stripped.Length - 4)
            );
        }
        else
            return String.Concat(S.Substring(0, 1), "*****", S.Substring(S.IndexOf("@") - 1)).ToLower();
    }
