 ...
 * @return {boolean}
 */
var escapeGhosts = function escapeGhosts(ghosts, target) {
  var steps = Math.abs(target[0]) + Math.abs(target[1]);
  var _iteratorNormalCompletion = true;
  var _didIteratorError = false;
  var _iteratorError = undefined;

  try {
    for (var _iterator = ghosts[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var ghost = _step.value;
      if (Math.abs(ghost[0] - target[0]) + Math.abs(ghost[1] - target[1]) <= steps) return false;
    }
  } catch (err) {
