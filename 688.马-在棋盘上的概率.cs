...
/*
 * @lc app=leetcode.cn id=688 lang=csharp
 *
 * [688] “马”在棋盘上的概率
 *
...
public class Solution
{
    ...
    {
        (int row, int column)[] moves = new (int, int)[] { (1, 2), (1, -2), (-1, -2), (-1, 2), (2, 1), (-2, -1), (2, -1), (-2, 1) };
            ...
            for (int i = 0; i < N; i++)
            {
                for (int j = 0; j < N; j++)
                {
                    foreach (var move in moves)
                    {
                        ...
                        int newColumn = j + move.column;

                        if (newRow >= 0 && newColumn >= 0 && newRow < N && newColumn < N)
                        {
                            d2[newRow, newColumn] += d1[i, j] / 8d;
                        }
                    }
                }
            }
