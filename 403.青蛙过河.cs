...
public class Solution {
    public bool CanCross(int[] stones) {
        ...
        foreach(int stone in stones) {
            dp.Add(stone, new HashSet<int>());
            if(stone == 0) {
                dp[stone].Add(0);
                continue;
            }
            foreach(var key in dp.Keys) {
                if(dp[key].Contains(stone - key)) dp[stone].Add(stone - key);
                if(dp[key].Contains(stone - key - 1)) dp[stone].Add(stone - key);
                if(dp[key].Contains(stone - key + 1)) dp[stone].Add(stone - key);
            }
        }
        return dp[stones.Last()].Any();
    }
}
// @lc code=end
