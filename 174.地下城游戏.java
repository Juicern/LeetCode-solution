...
class Solution {
    public int calculateMinimumHP(int[][] dungeon) {
        ...
        int[][] dp = new int[m][n];
        dp[m-1][n-1] = Integer.max(1, 1 - dungeon[m-1][n-1]);
        for(int i=m-2;i>=0;i--){
            dp[i][n-1] = Integer.max(1, dp[i+1][n-1] - dungeon[i][n-1]);
        }
        for(int i=n-2;i>=0;i--){
            dp[m-1][i] = Integer.max(1, dp[m-1][i+1] - dungeon[m-1][i]);
        }
        for(int i=m-2;i>=0;i--){
            for(int j=n-2;j>=0;j--){
                int dpmin = Integer.min(dp[i+1][j], dp[i][j+1]);
                dp[i][j] = Integer.max(1, dpmin - dungeon[i][j]);
            }
        }
        return dp[0][0];
    }
