...
public class Solution
{
    public bool CanIWin(int maxChoosableInteger, int desiredTotal)
    {
        ...
        var dp = new bool?[1 << maxChoosableInteger];
        return Helper(maxChoosableInteger, desiredTotal, dp, 0);
    }
    private bool Helper(int maxChoosableInteger, int desiredTotal, bool?[] dp, int state)
    {
        if (dp[state] != null) return dp[state].Value;
