...
/*
 * @lc app=leetcode.cn id=609 lang=csharp
 *
 * [609] 在系统中查找重复文件
 *
    ...
    public IList<IList<string>> FindDuplicate(string[] paths) {
        ...
        foreach(var path in paths) {
            ...
            for(int i = 1;i<strs.Length;i++) {
                ...
                string content = strs[i].Substring(left, right + 1 - left);
                if(!dict.ContainsKey(content)) dict.Add(content, new List<string>());
                dict[content].Add(strs[0] + "/"  + strs[i].Substring(0, left));
            }
        }
        var ans = new List<IList<string>>();
        foreach(var list in dict.Values){
            if(list.Count > 1) ans.Add(new List<string>(list));
        }
        return ans;
    }
}
